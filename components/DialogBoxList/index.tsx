                    ...
                    <header className="w-full">
                      <div
                        ...
                        <div
                          ...
                          {
                            model === OpenAIModel.GPT3_5.model && (
                              <span>Default ({OpenAIModel.GPT3_5.model})</span>
                            )
                          }
                          {
                            model === OpenAIModel.GPT4.model && (
                              <span>GPT-4 Model ({OpenAIModel.GPT4.model})</span>
                            )
                          }
                        </div>
                      </div>
                        ...
                        <ul className="flex w-full list-none gap-1 sm:w-auto">
                          <li className="group/toggle w-full">
                            <button type="button" className="w-full cursor-pointer" onClick={() => {
                              dispatch(setModel(OpenAIModel.GPT3_5.model))
                            }}>
                              <div
                                className={`group/button relative flex w-full items-center justify-center gap-1 rounded-lg border py-3 outline-none transition-opacity duration-100 sm:w-auto sm:min-w-[148px] md:gap-2 md:py-2.5 ${model === OpenAIModel.GPT3_5.model ? 'border-black/10 bg-white text-gray-900 shadow-[0_1px_7px_0px_rgba(0,0,0,0.06)] hover:!opacity-100 dark:border-[#4E4F60] dark:bg-gray-700 dark:text-gray-100' : 'border-transparent text-gray-500 hover:text-gray-800 hover:dark:text-gray-100'}`}>
                                <span className="relative max-[370px]:hidden">
                                  <svg xmlns="http://www.w3.org/2000/svg"
                                       viewBox="0 0 16 16" fill="none"
                                       className={`h-4 w-4 transition-colors ${model === OpenAIModel.GPT3_5.model ? 'text-brand-green' : ''} group-hover/button:text-brand-green`}
                                       width="16" height="16"
                                       strokeWidth="2"><path
                                ...
                                <span className="truncate text-sm font-medium md:pr-1.5 pr-1.5">{OpenAIModel.GPT3_5.topic}</span>
                                {
                                  !gpt3_5 && (
                          ...
                          <li className="group/toggle w-full">
                            <button type="button" className="w-full cursor-pointer" onClick={() => {
                              if (gpt4) {
                                dispatch(setModel(OpenAIModel.GPT4.model))
                              }
                            }} disabled={!gpt4}>
                              <div
                                className={`group/button relative flex w-full items-center justify-center gap-1 rounded-lg border py-3 outline-none transition-opacity duration-100 sm:w-auto sm:min-w-[148px] md:gap-2 md:py-2.5 ${model === OpenAIModel.GPT3_5.model ? 'border-transparent text-gray-500 hover:text-gray-800 hover:dark:text-gray-100' : 'border-black/10 bg-white text-gray-900 shadow-[0_1px_7px_0px_rgba(0,0,0,0.06)] hover:!opacity-100 dark:border-[#4E4F60] dark:bg-gray-700 dark:text-gray-100'}`}>
                                <span className="relative max-[370px]:hidden">
                                  <svg xmlns="http://www.w3.org/2000/svg"
                                       viewBox="0 0 16 16" fill="none"
                                       className={`h-4 w-4 transition-colors ${model === OpenAIModel.GPT4.model ? 'text-brand-purple' : ''} group-hover/button:text-brand-purple`}
                                       width="16" height="16"
                                       strokeWidth="2"><path
                                ...
                                <span className="truncate text-sm font-medium md:pr-1.5 pr-1.5">{OpenAIModel.GPT4.topic}</span>
                                {
                                  !gpt4 && (
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor"
                                         aria-hidden="true" strokeWidth="2"
